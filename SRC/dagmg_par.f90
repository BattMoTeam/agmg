...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_applyprec( N,f,X,a,ja,ia,flop)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL dagmgpar_prec_matv(1,f,X,S,flop,S(N+1),.FALSE.)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_csrdlu(n,a,ja,ia,idiag,ao,jao,il,iu,trans,iext,iexto)
!
!
!
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    !
    !
    USE dagmgpar_mem
    IMPLICIT NONE
    ...
    !
    IF (init==1) THEN
       CALL dagmgpar_matv(N,x,SD,a,ja,ia,flop,trans &
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL dagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),flop,S(1+5*N),.FALSE.)
       !
       !
       IF ( ITER > 1 ) THEN
       ...
       CALL dagmgpar_matv(N,S(1+2*N),SD,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0d0)) ::  flop
    ...
    !
    IF (init==1) THEN
       CALL dagmgpar_matv(N,x,Sc,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       f(1:n)=f(1:n)-Sc(1:N)
       dum(2) = DNRM2(N, f, IONE)**2
       dum(5:6)=dum(2:3)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL dagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),flop,R,.FALSE.)
...
!--------------------------------------------------------------------
  SUBROUTINE dagmgpar_matv(n, x, y, a, ja, ia, flop, transpose,         &
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE dagmgpar_prec_matv(l,B,X,AX,flop,R,matv)
!
!
!
!
...
!
    ...
    IF (n == 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          IF (.NOT.allzero)                                      &
               CALL dagmgpar_MUMPS(nnext,dum,2,flop)
          !
    ...
    IF (l == nlev) THEN
       ...
       !
       X(1:N)=B(1:N)
       CALL dagmgpar_MUMPS(n,X,2,flop)
       !
       RETURN
       !
    END IF
    ...
    IF (nsmotot == 1) THEN
       !
       CALL dagmgpar_fwGS(l,B,X,AX,flop,.FALSE.,R)
       !
    ELSE
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          CALL dagmgpar_fwbwsolve1(l,dt(l)%a,B,X,AX,flop,update,R)
          update=.TRUE.
       END DO
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL dagmgpar_fwGS(l,B,X,AX,flop,.TRUE.,R)
          !
       END IF
    END IF
    !
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL dagmgpar_MUMPS(nnext,R,2,flop)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
          RN=AXN+nnext
          CALL dagmgpar_prec_matv(l+1,R(BN),R(XN),R(AXN),flop,R(RN),.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
       ELSE
          !
          CALL dagmgpar_inner_iter(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
    ...
    ELSE
       ...
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          CALL dagmgpar_prec_matv(l+1,B,X,AX,flop,R,.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
    ...
    IF (nsmotot == 1) THEN
       !
       CALL dagmgpar_bwGS(l,B,X,AX,flop,matv)
       !
    ELSE
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL dagmgpar_bwGS(l,B,X,AX,flop,.FALSE.)
          !
       END IF
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          ...
          CALL dagmgpar_fwbwsolve2(l,dt(l)%a,B,X,AX,flop,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE dagmgpar_prec_matv
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_fwGS(l,B,X,AX,flop,update,R)
!
!
!
!
!
    ...
    ELSE
       !
       !
       CALL dagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,X,B,1,flop)
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_bwGS(l,B,X,AX,flop,matv)
!
!
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_fwbwsolve1(l,p,B,X,AX,flop,update,R)
!
!
!
!
...
!
    ...
    IF (.NOT.update) THEN
       !
       !
       CALL dagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,p    &
                            ,R,B,-1,flop)
       !
       !
       CALL dagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,p    &
                             ,X,R,1,flop)
       !
       !
       CALL dagmgpar_sendrec(X,idum,-1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_fwbwsolve2(l,p,B,X,AX,flop,matv,R)
!
!
!
!
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE dagmgpar_inner_iter(n,X,R,l,flop)
    !
    !
    !
    !
    ...
    !
    !
    CALL dagmgpar_prec_matv(l,R,X,R(1,2),flop,R(1,3),.TRUE.)
    !
    !
    ...
    !
    !
    CALL dagmgpar_prec_matv(l,R,R(1,3),R(1,4),flop,R(1,5),.TRUE.)
    !
    !
    !
...
!
    ...
    ELSE
       ...
       CALL dagmgpar_MUMPS(n,fff,1,ops,a,dt(1)%ja,ia)
       IF (wfo) THEN
       ...
       DEALLOCATE(dt(1)%ja)
       CALL dagmgpar_restorelstrank(dt(1)%ilstin,listrank)
       ifl=n+1
       !
...
!
    ...
    IF (n == 0) THEN
       ...
       ELSE IF (.NOT. allzero) THEN
          CALL dagmgpar_MUMPS(n,fff,1,ops,dt(l)%a,dt(l)%ja,dt(l)%ia)
       END IF
       !
       !
       RETURN
...
!
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL dagmgpar_findpairs_SI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       ELSE
          CALL dagmgpar_findpairs_GI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       END IF
       DEALLOCATE(iperm)
    ...
    ELSE
       IF (spd) THEN
          CALL dagmgpar_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       ELSE
          CALL dagmgpar_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       END IF
...
10  CONTINUE
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL dagmgpar_findpairs_SF(np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       ELSE
          CALL dagmgpar_findpairs_GF(np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       END IF
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_GF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_SF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent &
    ,iext,iext1)
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_GI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_SI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_GI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_findpairs_SI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_MUMPS(n,f,ijob,flop,a,ja,ia)
    USE dagmgpar_mem
    IMPLICIT NONE
!!!!!!!
...
!
    ...
    IF (preprocessed) THEN
       ...
       IF (.NOT.allzero)                        &
               CALL dagmgpar_MUMPS(nn(nlev),fdum,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
...
!
    ...
    CALL dagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,dt(1)%iext)
    DEALLOCATE(w,iw)
    !
      CALL dagmgpar_setsendrecL1(n,ja,ia,dt(1)%iext,listrank,ifirstlistrank)
    !
    !
...
300 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL dagmgpar_applyprec(n,f,x,a,ja,ia,flop)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL dagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    ELSE
   !
       CALL dagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    END IF
!
