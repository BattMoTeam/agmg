...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! This file provides cagmg and dependencies; cagmg is a sequential
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_applyprec( N,f,X,a,ja,ia,flop)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL cagmg_prec_matv(1,S,X,S(N+1),flop,S(2*N+1),.FALSE.)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_partroword(n, a, ja, ia, idiag, w, iw, iext)
!
!
!
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_csrdlu(n,a,ja,ia,idiag,ao,jao,il,iu,trans,iext,iexto)
!
!
!
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    !
    !
    USE cagmg_mem
    IMPLICIT NONE
    ...
    !
    IF (init==1) THEN
       CALL cagmg_matv(N,x,SD,a,ja,ia,flop,trans )
       f(1:n)=f(1:n)-SD(1:N)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL cagmg_prec_matv(1,fsc,S(1+3*N),S(1+4*N),flop,S(1+5*N),.FALSE.)
       ...
       CALL cagmg_matv(N,S(1+2*N),SD,a,ja,ia,flop,trans )
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0e0)) ::  flop
    ...
    !
    IF (init==1) THEN
       CALL cagmg_matv(N,x,Sc,a,ja,ia,flop,trans )
       f(1:n)=f(1:n)-Sc(1:N)
       dum(2) = SCNRM2(N, f, IONE)**2
       BNORM2=real(dum(3))
       RESID2=real(dum(2))
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       fsc(1:N)=f(1:N)*scald(1:N)
       CALL cagmg_prec_matv(1,fsc,Su(1+itm*N),Sc(1+itm*N)      &
            ,flop,R,.FALSE.)
...
!--------------------------------------------------------------------
  SUBROUTINE cagmg_matv(n, x, y, a, ja, ia, flop, transpose,         &
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmg_prec_matv(l,B,X,AX,flop,R,matv)
!
!
!
!
...
!
    ...
    IF (l == nlev) THEN
       ...
       !
       X(1:N)=B(1:N)
       CALL cagmg_MUMPSseq(n,X,2,flop)
       !
       RETURN
       !
    END IF
    ...
    IF (nsmotot == 1) THEN
       !
       CALL cagmg_fwGS(l,B,X,AX,flop,.FALSE.,R)
       !
    ELSE
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          CALL cagmg_fwbwsolve1(l,dt(l)%a,B,X,AX,flop,update,R)
          update=.TRUE.
       END DO
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL cagmg_fwGS(l,B,X,AX,flop,.TRUE.,R)
          !
       END IF
    END IF
    !
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL cagmg_MUMPSseq(nnext,R,2,flop)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
          RN=AXN+nnext
          CALL cagmg_prec_matv(l+1,R(BN),R(XN),R(AXN),flop,R(RN),.FALSE.)
          !
       ...
       ELSE
          !
          CALL cagmg_inner_iter(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
    ...
    IF (nsmotot == 1) THEN
       !
       CALL cagmg_bwGS(l,B,X,AX,flop,matv)
       !
    ELSE
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL cagmg_bwGS(l,B,X,AX,flop,.FALSE.)
          !
       END IF
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          ...
          CALL cagmg_fwbwsolve2(l,dt(l)%a,B,X,AX,flop,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE cagmg_prec_matv
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_fwGS(l,B,X,AX,flop,update,R)
!
!
!
!
!
    ...
    ELSE
       !
       !
       CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,X,B,1,flop)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_bwGS(l,B,X,AX,flop,matv)
!
!
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_fwbwsolve1(l,p,B,X,AX,flop,update,R)
!
!
!
!
...
!
    ...
    IF (.NOT.update) THEN
       !
       !
       CALL cagmg_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,p    &
                            ,R,B,-1,flop)
       !
       !
       CALL cagmg_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,p    &
                             ,X,R,1,flop)
       !
       !
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_fwbwsolve2(l,p,B,X,AX,flop,matv,R)
!
!
!
!
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmg_inner_iter(n,X,R,l,flop)
    !
    !
    !
    !
    ...
    !
    !
    CALL cagmg_prec_matv(l,R,X,R(1,2),flop,R(1,3),.TRUE.)
    !
    !
    ...
    !
    !
    CALL cagmg_prec_matv(l,R,R(1,3),R(1,4),flop,R(1,5),.TRUE.)
    !
    !
    !
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmg_setupL1(n,a,ja,ia,listrank,ifl)
!
!
    USE cagmg_mem
    IMPLICIT NONE
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL cagmg_MUMPSseq(n,fff,1,ops,a,ja,ia)
       IF (wfo) THEN
    ...
    !
       ...
       IF (wfo) THEN
          WRITE(iout,'()')
          WRITE(iout,954) nlctot(1)/real(nlc1(1))
          WRITE(iout,955) nlctot(2)/real(nlc1(2))
       END IF
       IF (wff) THEN
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL cagmg_MUMPSseq(n,fff,1,ops,dt(l)%a,dt(l)%ja,dt(l)%ia)
       !
...
!
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL cagmg_findpairs_SI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm )
       ELSE
          CALL cagmg_findpairs_GI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm )
       END IF
       DEALLOCATE(iperm)
       memi=memi-n
    ELSE
       IF (spd) THEN
          CALL cagmg_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm )
       ELSE
          CALL cagmg_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm )
       END IF
...
10  CONTINUE
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL cagmg_findpairs_SF(np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw )
       ELSE
          CALL cagmg_findpairs_GF(np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw )
       END IF
       DEALLOCATE(w,iw)
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_GF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_SF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent )
!
!
    USE cagmg_mem
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_GI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc )
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_SI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc )
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_GI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm )
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_findpairs_SI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm )
!
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_LAPACK(n,f,ijb,flop,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: n,ijb
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_MUMPSseq(n,f,ijob,flop,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
!!!!!!!
!
...
!
    ...
    IF (preprocessed) THEN
       CALL cagmg_relmem
       IF (.NOT.allzero)                        &
               CALL cagmg_MUMPSseq(nn(nlev),fdum,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0e0
       cputm=0.0e0
...
300 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL cagmg_applyprec(n,f,x,a,ja,ia,flop)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL cagmg_GCR(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    ELSE
   !
       CALL cagmg_FlexCG(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    END IF
!
