...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! This file provides sagmgpar and dependencies; sagmgpar is a parallel
! implementation for real matrices in single precision
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_applyprec( N,f,X,a,ja,ia,flop)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    INTEGER       :: ja(*), ia(N+1)
    REAL(kind(0.0e0)) :: a(*)
    REAL(kind(0.0e0)), ALLOCATABLE :: S(:)
    !
    mritr=nwrkcum+N
    ALLOCATE (S(nwrkcum+N))
    CALL sagmgpar_prec_matv(1,f,X,S,flop,S(N+1),.FALSE.)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_partroword(n, a, ja, ia, idiag, w, iw, iext)
!
!
!
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_csrdlu(n,a,ja,ia,idiag,ao,jao,il,iu,trans,iext,iexto)
!
!
!
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_sendrec(vect,ivect,ishift,lstout,ilstout,ilstin)
    USE sagmgpar_mem
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    !
    !
    USE sagmgpar_mem
    IMPLICIT NONE
    ...
    REAL(kind(0.0e0)) :: ALPHA, BET0, RHO, dum(6), td
    ...
    !
    IF (init==1) THEN
       CALL sagmgpar_matv(N,x,SD,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       f(1:n)=f(1:n)-SD(1:N)
       dum(2) = SNRM2(N, f, IONE)**2
       dum(5:6)=dum(2:3)
       CALL MPI_ALLREDUCE(dum(5),dum(2),2,MPI_REAL,  &
            MPI_SUM,ICOMM,ierr)
       BNORM=SQRT(dum(3))
       RESID=SQRT(dum(2))
       RESID0=RESID
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL sagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),flop,S(1+5*N),.FALSE.)
       !
       !
       IF ( ITER > 1 ) THEN
          dum(1) = - SDOT(N, SD, IONE, S(1+3*N), IONE)
          CALL MPI_ALLREDUCE(dum(1),BET0,1,MPI_REAL,    &
               MPI_SUM,ICOMM,ierr)
          BET1=BET0/RHO
       ...
       CALL sagmgpar_matv(N,S(1+2*N),SD,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       !
       ...
       IF (ITER==1) THEN
          dum(4:6)=dum(1:3)
          CALL MPI_ALLREDUCE(dum(4),dum(1),3,MPI_REAL,  &
               MPI_SUM,ICOMM,ierr)
          IF (init == 0) THEN
             BNORM=SQRT(dum(3))
             RESID0=BNORM
       ...
       ELSE
          dum(4:5)=dum(1:2)
          CALL MPI_ALLREDUCE(dum(4),dum(1),2,MPI_REAL,  &
               MPI_SUM,ICOMM,ierr)
       END IF
       !
       RHO=dum(1)
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0e0)) ::  flop
    ...
    !
    IF (init==1) THEN
       CALL sagmgpar_matv(N,x,Sc,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       f(1:n)=f(1:n)-Sc(1:N)
       dum(2) = SNRM2(N, f, IONE)**2
       dum(5:6)=dum(2:3)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL sagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),flop,R,.FALSE.)
       ...
       IF (RESID2 <= TOLT) THEN
          ...
          dum0 = RESID2
          CALL MPI_ALLREDUCE(dum0,RESID2,1,MPI_REAL,   &
               MPI_SUM,ICOMM,ierr)
          flop=flop+real(2*N)
          TOLT = MAX(TOL2BNORM2,TRS*RESID2)
       END IF
...
!--------------------------------------------------------------------
  SUBROUTINE sagmgpar_matv(n, x, y, a, ja, ia, flop, transpose,         &
...
!
    ...
    LOGICAL :: transpose
    !
    CALL sagmgpar_sendrec(x,idum,-1,lstout,ilstout,ilstin)
    !
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE sagmgpar_prec_matv(l,B,X,AX,flop,R,matv)
!
!
!
!
    ...
    REAL(kind(0.0e0)), OPTIONAL ::  B(*), X(*), AX(*), R(*)
...
!
    ...
    IF (n == 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          IF (.NOT.allzero)                                      &
               CALL sagmgpar_MUMPS(nnext,dum,2,flop)
          !
    ...
    IF (l == nlev) THEN
       ...
       !
       X(1:N)=B(1:N)
       CALL sagmgpar_MUMPS(n,X,2,flop)
       !
       RETURN
       !
    END IF
    ...
    IF (nsmotot == 1) THEN
       !
       CALL sagmgpar_fwGS(l,B,X,AX,flop,.FALSE.,R)
       !
    ELSE
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          CALL sagmgpar_fwbwsolve1(l,dt(l)%a,B,X,AX,flop,update,R)
          update=.TRUE.
       END DO
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL sagmgpar_fwGS(l,B,X,AX,flop,.TRUE.,R)
          !
       END IF
    END IF
    !
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL sagmgpar_MUMPS(nnext,R,2,flop)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
          RN=AXN+nnext
          CALL sagmgpar_prec_matv(l+1,R(BN),R(XN),R(AXN),flop,R(RN),.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
       ELSE
          !
          CALL sagmgpar_inner_iter(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
    ...
    ELSE
       ...
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          CALL sagmgpar_prec_matv(l+1,B,X,AX,flop,R,.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
    ...
    IF (nsmotot == 1) THEN
       !
       CALL sagmgpar_bwGS(l,B,X,AX,flop,matv)
       !
    ELSE
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL sagmgpar_bwGS(l,B,X,AX,flop,.FALSE.)
          !
       END IF
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          ...
          CALL sagmgpar_fwbwsolve2(l,dt(l)%a,B,X,AX,flop,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE sagmgpar_prec_matv
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_fwGS(l,B,X,AX,flop,update,R)
!
!
!
!
!
    ...
    IF (update) THEN
       ...
       CALL sagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a   &
                            ,R,AX,1,flop)
       CALL sagmgpar_sendrec(R,idum, -1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
       X(1:n)=X(1:n)+R(1:n)
       flop=flop+real(n)
       !
       !
    ...
    ELSE
       !
       !
       CALL sagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,X,B,1,flop)
       CALL sagmgpar_sendrec(X,idum,-1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_bwGS(l,B,X,AX,flop,matv)
!
!
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_fwbwsolve1(l,p,B,X,AX,flop,update,R)
!
!
!
!
...
!
    ...
    IF (.NOT.update) THEN
       !
       !
       CALL sagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,p    &
                            ,R,B,-1,flop)
       !
       !
       CALL sagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,p    &
                             ,X,R,1,flop)
       !
       !
       CALL sagmgpar_sendrec(X,idum,-1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
    ...
    ELSE
       ...
       !
       !
       CALL sagmgpar_sendrec(R(n+1),idum,-1,dt(l)%lstout   &
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_fwbwsolve2(l,p,B,X,AX,flop,matv,R)
!
!
!
!
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE sagmgpar_inner_iter(n,X,R,l,flop)
    !
    !
    !
    !
    ...
    !
    !
    CALL sagmgpar_prec_matv(l,R,X,R(1,2),flop,R(1,3),.TRUE.)
    !
    !
    ...
    dum(6:10)=dum(1:5)
    CALL MPI_ALLREDUCE(dum(6),dum(1),5,MPI_REAL,       &
         MPI_SUM,ICOMM,ierr)
    !
    !
    ...
    !
    !
    CALL sagmgpar_prec_matv(l,R,R(1,3),R(1,4),flop,R(1,5),.TRUE.)
    !
    !
    !
    ...
    dum(5:8)=dum(1:4)
    CALL MPI_ALLREDUCE(dum(5),dum(1),4,MPI_REAL,       &
         MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
    rho2  =dum(3)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE sagmgpar_setupL1(n,a,ja,ia,listrank,ifl)
!
!
    USE sagmgpar_mem
    IMPLICIT NONE
    ...
    ELSE
       ...
       CALL sagmgpar_MUMPS(n,fff,1,ops,a,dt(1)%ja,ia)
       IF (wfo) THEN
       ...
       DEALLOCATE(dt(1)%ja)
       CALL sagmgpar_restorelstrank(dt(1)%ilstin,listrank)
       ifl=n+1
       !
...
!
    ...
    IF (n == 0) THEN
       ...
       ELSE IF (.NOT. allzero) THEN
          CALL sagmgpar_MUMPS(n,fff,1,ops,dt(l)%a,dt(l)%ja,dt(l)%ia)
       END IF
       !
       !
       RETURN
...
!
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL sagmgpar_findpairs_SI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       ELSE
          CALL sagmgpar_findpairs_GI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       END IF
       DEALLOCATE(iperm)
    ...
    ELSE
       IF (spd) THEN
          CALL sagmgpar_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       ELSE
          CALL sagmgpar_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       END IF
...
10  CONTINUE
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL sagmgpar_findpairs_SF(np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       ELSE
          CALL sagmgpar_findpairs_GF(np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       END IF
...
999 CONTINUE
    ...
    ALLOCATE (jan(isize))
    memi=memi+isize
    CALL sagmgpar_setextnum(nc,l,listrank,jan)
    !
    IF (nc == 0) RETURN
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_GF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_SF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent &
    ,iext,iext1)
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_GI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_SI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_GI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_findpairs_SI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_setsendrecL1(n,ja,ia,iext,listrank,ifl)
    USE sagmgpar_mem
    IMPLICIT NONE
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_setextnum(nc,l,listrank,iw)
    USE sagmgpar_mem
    IMPLICIT NONE
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_restorelstrank(ilstin,listrank)
    USE sagmgpar_mem
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_MUMPS(n,f,ijob,flop,a,ja,ia)
    USE sagmgpar_mem
    IMPLICIT NONE
!!!!!!!
...
!!!!!!!
    ...
    !
    RETURN
  END SUBROUTINE sagmgpar_MUMPS
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_globnum(n,ja,ia,iext,irg,jcg,lstout         &
    ...
    END DO
    !
    CALL sagmgpar_sendrec(dum,irg,ishift,lstout,ilstout,ilstin)
    !
    DO i=1,n
       DO k=ia(i),iext(i)-1
          irg(k)=i+ishift
...
!
    IF (.NOT.preprocessed) CALL sagmgpar_init(MPI_COMM_AGMG)
    wff=wfo.AND.(IRANK<=0)
!
...
!
    ...
    IF (preprocessed) THEN
       CALL sagmgpar_relmem
       IF (.NOT.allzero)                        &
               CALL sagmgpar_MUMPS(nn(nlev),fdum,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0e0
       cputm=0.0e0
...
!
    IF (HUGE(n) > 1.0e10) THEN
       rlenilen=real(8)/real(real_len)
    ELSE
       rlenilen=real(4)/real(real_len)
    END IF
!
!
!
...
!
    ...
    CALL sagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,dt(1)%iext)
    DEALLOCATE(w,iw)
    !
      CALL sagmgpar_setsendrecL1(n,ja,ia,dt(1)%iext,listrank,ifirstlistrank)
    !
    !
...
300 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL sagmgpar_applyprec(n,f,x,a,ja,ia,flop)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL sagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    ELSE
   !
       CALL sagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    END IF
!
