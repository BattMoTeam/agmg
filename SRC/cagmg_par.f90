...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! This file provides cagmgpar and dependencies; cagmgpar is a parallel
! implementation for complex matrices in single precision
...
!         modified matrix with mean row and colum sum enforced to be zero).
    REAL(kind(0.0e0)), PARAMETER :: resi=0.2, trspos=0.45
    REAL(kind(0.0e0)), PARAMETER :: kaptg_blocdia=8, kaptg_dampJac=10
...
!
    ...
    REAL(kind(0.0e0)) :: memi=0.0,memax=0.0,memr=0.0,mritr,rlenilen
    REAL(kind(0.0e0)) :: wcplex(4),fracnz(maxlev)
    LOGICAL :: spd,wfo,wff,allzero,trans,transint,zerors,gcrin
    REAL(kind(0.0e0)), PARAMETER :: cplxmax=3.0, xsi=0.6d0
    REAL(kind(0.0e0)), PARAMETER :: repsmach=SQRT(EPSILON(1.0e0))
    INTEGER :: nlc(2),nlcp(2),nlc1(2),icum,imult
    REAL(kind(0.0e0)) :: ngl(2),nglp(2),nlctot(2),ngl1(2),ngltot(2)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_applyprec( N,f,X,a,ja,ia,flop)
    USE cagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    COMPLEX(kind(0.0e0)), ALLOCATABLE :: S(:)
    !
    mritr=nwrkcum+2*N
    ALLOCATE (S(nwrkcum+2*N))
    S(1:N)=f(1:n)*scald(1:N)
    CALL cagmgpar_prec_matv(1,S,X,S(N+1),flop,S(2*N+1),.FALSE.)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_partroword(n, a, ja, ia, idiag, w, iw, iext)
!
!
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_diagscal(n,a,ja,ia,idiag,as)
    ...
    END DO
    CALL cagmgpar_sendrec(scald,idum,-1,dt(1)%lstout,dt(1)%ilstout,dt(1)%ilstin)
    DO i=1,n
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_csrdlu(n,a,ja,ia,idiag,ao,jao,il,iu,trans,iext,iexto)
!
!
!
!
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_sendrec(vect,ivect,ishift,lstout,ilstout,ilstin)
    USE cagmgpar_mem
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    !
    !
    USE cagmgpar_mem
    IMPLICIT NONE
    ...
    COMPLEX(kind(0.0e0)) :: ALPHA, BET0, RHO, dum(6), td
    ...
    !
    IF (init==1) THEN
       CALL cagmgpar_matv(N,x,SD,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       f(1:n)=f(1:n)-SD(1:N)
       dum(2) = SCNRM2(N, f, IONE)**2
       dum(5:6)=dum(2:3)
       CALL MPI_ALLREDUCE(dum(5),dum(2),2,MPI_SINGLE_COMPLEX,  &
            MPI_SUM,ICOMM,ierr)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL cagmgpar_prec_matv(1,fsc,S(1+3*N),S(1+4*N),flop,S(1+5*N),.FALSE.)
       ...
       IF ( ITER > 1 ) THEN
          ...
          CALL MPI_ALLREDUCE(dum(1),BET0,1,MPI_SINGLE_COMPLEX,    &
               MPI_SUM,ICOMM,ierr)
          BET1=BET0/RHO
       ...
       CALL cagmgpar_matv(N,S(1+2*N),SD,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       !
       ...
       IF (ITER==1) THEN
          dum(4:6)=dum(1:3)
          CALL MPI_ALLREDUCE(dum(4),dum(1),3,MPI_SINGLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
          IF (init == 0) THEN
             BNORM=SQRT(real(dum(3)))
             RESID0=BNORM
       ...
       ELSE
          dum(4:5)=dum(1:2)
          CALL MPI_ALLREDUCE(dum(4),dum(1),2,MPI_SINGLE_COMPLEX,  &
               MPI_SUM,ICOMM,ierr)
       END IF
       !
       RHO=dum(1)
       ...
       CALL MPI_ALLREDUCE(dum0,RESID,1,MPI_REAL,       &
            MPI_SUM,ICOMM,ierr)
       RESID=SQRT(RESID)
       RESID=RESID/BNORM
       IF (wff) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init,flop)
    USE cagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0e0)) ::  flop
    ...
    !
    IF (init==1) THEN
       CALL cagmgpar_matv(N,x,Sc,a,ja,ia,flop,trans &
            ,iextL1,dt(1)%lstout,dt(1)%ilstout,dt(1)%lstin,dt(1)%ilstin)
       f(1:n)=f(1:n)-Sc(1:N)
       dum(2) = SCNRM2(N, f, IONE)**2
       dum(5:6)=dum(2:3)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       fsc(1:N)=f(1:N)*scald(1:N)
       CALL cagmgpar_prec_matv(1,fsc,Su(1+itm*N),Sc(1+itm*N)      &
            ,flop,R,.FALSE.)
       ...
       IF (RESID2 <= TOLT) THEN
          ...
          dum0 = RESID2
          CALL MPI_ALLREDUCE(dum0,RESID2,1,MPI_SINGLE_COMPLEX,   &
               MPI_SUM,ICOMM,ierr)
          flop=flop+real(2*N)
          TOLT = MAX(TOL2BNORM2,TRS*RESID2)
       END IF
...
!--------------------------------------------------------------------
  SUBROUTINE cagmgpar_matv(n, x, y, a, ja, ia, flop, transpose,         &
...
!
    ...
    LOGICAL :: transpose
    !
    CALL cagmgpar_sendrec(x,idum,-1,lstout,ilstout,ilstin)
    !
    ...
    END DO
    !
    flop = flop + real(2 *(ia(n+1)-1)-n)
    !
    RETURN
  END SUBROUTINE cagmgpar_matv
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmgpar_prec_matv(l,B,X,AX,flop,R,matv)
!
!
!
!
    ...
    IMPLICIT NONE
    INTEGER :: l
    REAL(kind(0.0e0)), OPTIONAL :: flop
    COMPLEX(kind(0.0e0)), OPTIONAL ::  B(*), X(*), AX(*), R(*)
...
!
    ...
    IF (n == 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          IF (.NOT.allzero)                                      &
               CALL cagmgpar_MUMPS(nnext,dum,2,flop)
          !
    ...
    IF (l == nlev) THEN
       ...
       !
       X(1:N)=B(1:N)
       CALL cagmgpar_MUMPS(n,X,2,flop)
       !
       RETURN
       !
    END IF
    ...
    IF (nsmotot == 1) THEN
       !
       CALL cagmgpar_fwGS(l,B,X,AX,flop,.FALSE.,R)
       !
    ELSE
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          CALL cagmgpar_fwbwsolve1(l,dt(l)%a,B,X,AX,flop,update,R)
          update=.TRUE.
       END DO
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL cagmgpar_fwGS(l,B,X,AX,flop,.TRUE.,R)
          !
       END IF
    END IF
    !
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL cagmgpar_MUMPS(nnext,R,2,flop)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
          RN=AXN+nnext
          CALL cagmgpar_prec_matv(l+1,R(BN),R(XN),R(AXN),flop,R(RN),.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
       ELSE
          !
          CALL cagmgpar_inner_iter(nnext,R(XN),R(BN),l+1,flop)
          !
       END IF
       !
    ...
    ELSE
       ...
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          CALL cagmgpar_prec_matv(l+1,B,X,AX,flop,R,.FALSE.)
          !
          kstat(1,l+1)=MAX(kstat(1,l+1),1)
          kstat(2,l+1)=kstat(2,l+1)+1
          !
    ...
    IF (nsmotot == 1) THEN
       !
       CALL cagmgpar_bwGS(l,B,X,AX,flop,matv)
       !
    ELSE
       IF (mod(nsmotot,2) .EQ. 1) THEN
          !
          CALL cagmgpar_bwGS(l,B,X,AX,flop,.FALSE.)
          !
       END IF
       !
       update=.FALSE.
       DO is=2,nsmotot,2
          ...
          CALL cagmgpar_fwbwsolve2(l,dt(l)%a,B,X,AX,flop,update,R)
       END DO
    END IF
    RETURN
  END SUBROUTINE cagmgpar_prec_matv
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_fwGS(l,B,X,AX,flop,update,R)
!
!
!
!
!
    ...
    IF (update) THEN
       ...
       CALL cagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a   &
                            ,R,AX,1,flop)
       CALL cagmgpar_sendrec(R,idum, -1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
       X(1:n)=X(1:n)+R(1:n)
       flop=flop+real(n)
       !
       !
    ...
    ELSE
       !
       !
       CALL cagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,dt(l)%a,X,B,1,flop)
       CALL cagmgpar_sendrec(X,idum,-1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
       !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_bwGS(l,B,X,AX,flop,matv)
!
!
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_fwbwsolve1(l,p,B,X,AX,flop,update,R)
!
!
!
!
...
!
    ...
    IF (.NOT.update) THEN
       !
       !
       CALL cagmgpar_csrlsolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%il,p    &
                            ,R,B,-1,flop)
       !
       !
       CALL cagmgpar_csrusolve(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iu,p    &
                             ,X,R,1,flop)
       !
       !
       CALL cagmgpar_sendrec(X,idum,-1,dt(l)%lstout,dt(l)%ilstout,dt(l)%ilstin)
    ...
    ELSE
       ...
       !
       !
       CALL cagmgpar_sendrec(R(n+1),idum,-1,dt(l)%lstout   &
       ...
       IF (smoothtype == 2) flop=flop+real(2*n)
       IF (nneighi > 0) CALL MPI_WAITALL(nneighi,ireqi,ISTATUS,ier)
       CALL  cagmgpar_csrmv(n,dt(l)%a,dt(l)%ja,n+1,dt(l)%iext,buffi,n+1,AX   &
                         ,-2,flop,dt(l)%lstin)
    END IF
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_fwbwsolve2(l,p,B,X,AX,flop,matv,R)
!
!
!
!
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmgpar_inner_iter(n,X,R,l,flop)
    !
    !
    !
    !
    ...
    REAL(kind(0.0e0)) ::  RESID, flop, BNORM, det
    COMPLEX(kind(0.0e0)) :: X(N), R(N,*)
    COMPLEX(kind(0.0e0)) :: alpha1,alpha2,bet0,bet1,bet2,rho1,rho2,gamm0,gamm1,zeta
    COMPLEX(kind(0.0e0)) :: dum(10),y1,y2
    COMPLEX(kind(0.0e0)), external :: CDOTC
    REAL(kind(0.0e0)), external :: SCNRM2
    INTEGER , parameter :: IONE=1
    !
    ...
    !
    !
    CALL cagmgpar_prec_matv(l,R,X,R(1,2),flop,R(1,3),.TRUE.)
    !
    !
    ...
    dum(6:10)=dum(1:5)
    CALL MPI_ALLREDUCE(dum(6),dum(1),5,MPI_SINGLE_COMPLEX,       &
         MPI_SUM,ICOMM,ierr)
    !
    !
    ...
    !
    !
    CALL cagmgpar_prec_matv(l,R,R(1,3),R(1,4),flop,R(1,5),.TRUE.)
    !
    !
    !
    ...
    dum(5:8)=dum(1:4)
    CALL MPI_ALLREDUCE(dum(5),dum(1),4,MPI_SINGLE_COMPLEX,       &
         MPI_SUM,ICOMM,ierr)
    gamm0 =dum(1)
    alpha2=dum(2)
    rho2  =dum(3)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmgpar_inner_fool(l)
    ...
    IMPLICIT NONE
    INTEGER       :: ITER, l, ierr
    COMPLEX(kind(0.0e0)):: dum(10)
    REAL(kind(0.0e0)) :: BNORM,dum0,RESID,alpha1,rho1,bet0
    !
    !
    ITER = 1
    !
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmgpar_setupL1(n,a,ja,ia,listrank,ifl)
!
!
    USE cagmgpar_mem
    IMPLICIT NONE
    ...
    !
    dum=nlc
    CALL MPI_ALLREDUCE(dum,ngl,2,MPI_REAL,            &
         MPI_SUM,ICOMM,ierr)
       !
       wcplex=1.0e0
       nlctot=nlc
       ...
       allzero=.FALSE.
       nglp=0.0e0
       maxcoarset=FLOOR(maxcoarset*(ngl(1)**(1.0e0/3)))
       maxcoarseslowt=FLOOR(maxcoarseslowt*(ngl(1)**(1.0e0/3)))
       !
       IF (wfo) THEN
          IF(wff) THEN
    ...
    ELSE
       ...
       CALL cagmgpar_MUMPS(n,fff,1,ops,a,dt(1)%ja,ia)
       IF (wfo) THEN
          WRITE(iout,911) IRANK,ops/real(2*nlc1(2))
       END IF
       wcplex(4)=0.0e0
       IF (ngl(1) > maxcoarset)   wcplex(4)=-1.0e0
       DEALLOCATE(dt(1)%ja)
       CALL cagmgpar_restorelstrank(dt(1)%ilstin,listrank)
       ifl=n+1
       !
       IF (ngl(1) > maxcoarseslowt) wcplex(4)=ngl(1)/(1000*ngl1(1)**(1.0e0/3))
       !
       !
       RETURN
    END IF
    ...
    !
       ...
       END DO
             ...
             END IF
             IF (wfo) THEN
                WRITE(iout,952)  IRANK,nlctot(1)/real(nlc1(1))
                WRITE(iout,953)  IRANK,nlctot(2)/real(nlc1(2))
             END IF
       IF (wff) THEN
...
!
    ...
    REAL(kind(0.0e0)) :: ops,fw,eta,dum(2)
    CHARACTER(len=13) :: prtint
    COMPLEX (kind(0.0e0)) :: fff(1)
    !
    nn(l)=n
    ...
    IF (n == 0) THEN
       ...
       ELSE IF (.NOT. allzero) THEN
          CALL cagmgpar_MUMPS(n,fff,1,ops,dt(l)%a,dt(l)%ja,dt(l)%ia)
       END IF
       !
       !
       RETURN
...
!
    ...
    IF (l > 1) THEN
       IF (spd) THEN
          CALL cagmgpar_findpairs_SI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       ELSE
          CALL cagmgpar_findpairs_GI(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,iperm &
               ,dt(l)%iext)
       END IF
       DEALLOCATE(iperm)
    ...
    ELSE
       IF (spd) THEN
          CALL cagmgpar_findpairs_SI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       ELSE
          CALL cagmgpar_findpairs_GI1(n,a,ja,ia,dt(l)%idiag,si1  &
               ,ind2,lcg,nc,ndd,ldd,skipass,riperm,iperm &
               ,dt(l)%iext)
       END IF
...
10  CONTINUE
    ...
    DO kpass=2,npass1
       ...
       ind2(1:np)=-1
       IF (spd) THEN
          CALL cagmgpar_findpairs_SF(np,ap,jap,iap,idiagp,sip  &
               ,ind2,lcg,nc                                  &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       ELSE
          CALL cagmgpar_findpairs_GF(np,ap,jap,iap,idiagp,sip     &
               ,ind2,lcg,nc                                     &
               ,m1,lcg1,a,ja,ia,dt(l)%idiag,si1,w,iw &
               ,iextp,dt(l)%iext)
       END IF
...
999 CONTINUE
    ...
    ALLOCATE (jan(isize))
    memi=memi+isize
    CALL cagmgpar_setextnum(nc,l,listrank,jan)
    !
    IF (nc == 0) RETURN
    !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_GF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
...
!
    ...
    REAL(kind(0.0e0)) :: alpha, alp, tmp, beta, f1, f2
...
!
    DO j=1,SetDim
       DO k=1,SetDim
          ...
          END IF
       ENDDO
       IF (sig(j) < 0.0e0)  AGe(j)=AGe(j)+2*sig(j)
   !
       v(j)=W(j,j)
   !
   !
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_SF(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,m1,lcg1,a1,ja1,ia1,idiag1,si1,rtent,jtent &
    ,iext,iext1)
!
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_GI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_SI(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,ipc &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_GI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_findpairs_SI1(n,a,ja,ia,idiag,si,ind,lcg,nc     &
    ,nddl,ldd,skipass                                      &
    ,riperm,iperm &
    ,iext)
!
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_setsendrecL1(n,ja,ia,iext,listrank,ifl)
    USE cagmgpar_mem
    IMPLICIT NONE
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_setextnum(nc,l,listrank,iw)
    USE cagmgpar_mem
    IMPLICIT NONE
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_restorelstrank(ilstin,listrank)
    USE cagmgpar_mem
    IMPLICIT NONE
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_MUMPS(n,f,ijob,flop,a,ja,ia)
    USE cagmgpar_mem
    IMPLICIT NONE
!!!!!!!
...
!!!!!!!
    ...
    !
    RETURN
  END SUBROUTINE cagmgpar_MUMPS
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_globnum(n,ja,ia,iext,irg,jcg,lstout         &
    ...
    END DO
    !
    CALL cagmgpar_sendrec(dum,irg,ishift,lstout,ilstout,ilstin)
    !
    DO i=1,n
       DO k=ia(i),iext(i)-1
          irg(k)=i+ishift
...
!
    INTEGER :: nza
    REAL(kind(0.0e0)), SAVE :: cputm=0.0e0,eltm=0.0e0,flop=0.0e0
    LOGICAL, SAVE :: preprocessed=.FALSE.,solve=.FALSE.
    INTEGER :: i,init,ijb,k
    REAL(kind(0.0e0)) :: cputmp,eltmp,memh
    ...
    COMPLEX(kind(0.0e0)) :: adum(1),fdum(1)
!
    wfo=.TRUE.
    iout=iprint
    IF (iprint <= 0) THEN
...
!
    IF (.NOT.preprocessed) CALL cagmgpar_init(MPI_COMM_AGMG)
    wff=wfo.AND.(IRANK<=0)
!
...
!
    ...
    IF (preprocessed) THEN
       CALL cagmgpar_relmem
       IF (.NOT.allzero)                        &
               CALL cagmgpar_MUMPS(nn(nlev),fdum,-2,flop)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0e0
       cputm=0.0e0
...
!
    IF (HUGE(n) > 1.0e10) THEN
       rlenilen=real(8)/real(complex_len)
    ELSE
       rlenilen=real(4)/real(complex_len)
    END IF
!
!
!
...
!
    ...
    CALL cagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,dt(1)%iext)
    DEALLOCATE(w,iw)
    !
      CALL cagmgpar_setsendrecL1(n,ja,ia,dt(1)%iext,listrank,ifirstlistrank)
    !
    !
    !
...
300 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL cagmgpar_applyprec(n,f,x,a,ja,ia,flop)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL cagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    ELSE
   !
       CALL cagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init,flop)
   !
   !
    END IF
!
